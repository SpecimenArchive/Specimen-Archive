...
export class Storage {
  ...
  record(snapshot:Snapshot){appendFileSync(this.path(this.session.id+'.jsonl'),JSON.stringify(snapshot)+'\n');this.session.frames++;this.session.duration=snapshot.modelTime;}
  checkpoint(engine:Engine){this.atomic('checkpoint.json',{savedAt:new Date().toISOString(),modelVersion:MODEL_CONFIG.version,datasetVersion:engine.circuit.version,state:engine.checkpoint()});this.writeMeta();}
  restore(engine:Engine){const file=this.path('checkpoint.json');if(!existsSync(file))return;try{const c=JSON.parse(readFileSync(file,'utf8'));if(c.modelVersion!==MODEL_CONFIG.version||c.datasetVersion!==engine.circuit.version||c.state.activity.length!==engine.network.activity.length)throw new Error('Incompatible checkpoint');engine.restore(c.state);this.session.recovered=true;engine.event('session',`Recovered checkpoint; wall-time gap ${Math.max(0,(Date.now()-Date.parse(c.savedAt))/1000).toFixed(1)} s · no catch-up`);}catch(e){engine.event('session',`Checkpoint unavailable: ${String(e).slice(0,90)}`);}}
