...
server.on('request',(req,res)=>{
  ...
  if(url.pathname==='/api/exhibit/publications'){json(exhibitService.publications().slice(0,200));return;}
  if(url.pathname==='/api/exhibit/records'){const records=exhibitService.records();const selected=[...new Map([...records.slice(0,200),...records.filter(r=>r.config.heldOutSeeds.includes(r.seed))].map(r=>[r.id,r])).values()];json(selected.map(({decisions,observationEvents,...r})=>({...r,decisions:decisions.length,actions:decisions.filter(d=>d.command.kind!=='wait').length,executedActions:observationEvents?observationEvents.filter(e=>e.source==='neural'&&e.kind!=='wait'&&e.status==='completed'&&e.result?.scrollAfter!==e.result?.scrollBefore).length:null,artifactsAvailable:!!exhibitService.file(r.id,'trace.json.gz')})));return;}
