...
server.on('request',(req,res)=>{
  ...
  if(url.pathname==='/api/exhibit/records'){void exhibitService.archive().then(({records})=>{const selected=[...new Map([...records.slice(0,200),...records.filter(r=>r.config.heldOutSeeds.includes(r.seed))].map(r=>[r.id,r])).values()];json(selected.map(({decisions,observationEvents,displayFrames,initialCheckpoint,artifacts,setup,...r})=>({...r,setup:{note:setup.note,events:[]},artifacts:artifacts.filter(a=>/\.webm$/.test(a.path)),decisions:decisions.length,actions:decisions.filter(d=>d.command.kind!=='wait').length,executedActions:observationEvents?observationEvents.filter(e=>e.source==='neural'&&e.kind!=='wait'&&e.status==='completed'&&e.result?.scrollAfter!==e.result?.scrollBefore).length:null,artifactsAvailable:!!exhibitService.file(r.id,'trace.json.gz')})));}).catch(()=>json({error:'Archive unavailable'},503));return;}
