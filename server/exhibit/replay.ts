...
export async function replayEpisode(directory:string,circuit:Circuit){
  ...
  if(r.externalConfig)assert.deepEqual(r.externalConfig,EXTERNAL_POLICY);
  ...
  if(r.initialCheckpoint){assert(r.externalConfig);controller.engine.restore(r.initialCheckpoint);}
  for(const d of trace){
    ...
    if(d.sensoryProfile){assert.equal(d.sensoryProfile,r.observationConfig?.retina.version??OBSERVATION_RETINA.version);assert(r.observationConfig||r.externalConfig);assert(d.page?.id);}
    ...
    if(r.externalConfig){
      assert(d.receipt);assert(['scroll','wait'].includes(d.command.kind));assert.equal(inputs.length,d.receipt.trustedEvents);
      if(['policy-blocked','stale-input'].includes(d.receipt.status)){assert.equal(inputs.length,0);assert(!d.receipt.dispatchedAt);previous=d;continue;}
      if(d.receipt.status==='moved')assert.notEqual(d.receipt.scrollAfter,d.receipt.scrollBefore);
      if(d.receipt.status==='boundary')assert.equal(d.receipt.scrollAfter,d.receipt.scrollBefore);
    }
