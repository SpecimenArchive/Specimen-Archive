...
export class ExternalStation {
 ...
 private retryLimit:number=P.maximumUnchanged;private recalledVisit='';
 constructor(readonly journal:ObservationJournal,readonly memory?:ExperienceStore){}
 ...
 async episode(circuit:Circuit,o:EpisodeOptions){
  ...
  try{
   ...
   for(let decision=0;decision<P.decisionWindows;decision++){
    ...
    if(this.encounterDecisions>=P.maximumEncounterDecisions||this.unchanged>=this.retryLimit){await this.discover();const next=this.planner.next(this.encounter!.url);if(next===this.encounter!.url){this.operation('stagnation','No new approved coin links are available; visiting published larval research.','blocked');await this.navigate(RESEARCH_PAGES[0],'No new approved listing destination.');}else await this.navigate(next,this.unchanged>=this.retryLimit?`Recovered after ${this.retryLimit} unchanged results. Retry policy is supervisor-owned.`:'Encounter observation budget reached.');this.retryLimit=P.maximumUnchanged;}
  ...
  finally{if(record.incompleteAction)record.incompleteAction.events=this.events.filter(e=>e.commandId===record.incompleteAction!.commandId);this.commandId=null;await this.stopDisplay();this.journal.cancelRun(runId,record.error??'Recording segment ended');await this.memory?.flush();}
