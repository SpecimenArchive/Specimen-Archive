...
import type {LiveJournal} from '../journal/store';import type {SemanticNarrator} from '../journal/narrator';import type {ResearchProposal,NarrationContext} from '../../shared/journal';
...
export class ExternalStation {
 ...
 private planner:ResearchPlanner;private encounter?:Encounter;private journey:Encounter[]=[];private visit=0;private encounterDecisions=0;private unchanged=0;
 ...
 private retryLimit:number=P.maximumUnchanged;private recalledVisit='';private pageText='';private narratedVisit='';
 constructor(readonly journal:ObservationJournal,readonly memory?:ExperienceStore,readonly publicJournal?:LiveJournal,readonly narrator?:SemanticNarrator,runtime='runtime'){this.planner=new ResearchPlanner(runtime,memory,publicJournal);if(process.env.EXHIBIT_EXTERNAL_ENVIRONMENT==='research')this.planner.ponsBlocked='Research environment configured.';}
 exploration(){return this.planner.view();}
 ...
 private async open(){
  ...
  await this.context.route('**/*',async route=>{
   const r=route.request(),u=new URL(r.url());const pons=u.origin==='https://www.ponsfamily.com',research=u.hostname==='elifesciences.org'||u.hostname.endsWith('.elifesciences.org')||u.hostname==='jekelylab.github.io'||u.hostname==='ethereum.org'||u.hostname==='eth.blockscout.com';
 ...
 private async navigate(url:string,reason:string,proposal?:ResearchProposal){
  ...
  if(proposal&&this.encounter){this.publicJournal?.transition(proposal,'accepted',this.encounter,event.id);this.planner.accepted(proposal,this.encounter,event.id);}
  let response;try{response=await this.page!.goto(url,{waitUntil:'domcontentloaded',timeout:25000});}catch(error){this.planner.result(url,'failed');this.journal.finish(event,{status:'failed',reason:'Approved navigation timed out or failed.',completedAt:new Date().toISOString()});if(proposal&&this.encounter)this.publicJournal?.transition(proposal,'failed',this.encounter,event.id);throw error;}await this.page!.waitForTimeout(700);
  ...
  this.pageText=text;
  if(response&&response.status()>=400&&!restriction){this.planner.result(url,'failed');this.journal.finish(event,{status:'failed',reason:`Navigation returned HTTP ${response.status()}`,completedAt:new Date().toISOString()});if(proposal&&this.encounter)this.publicJournal?.transition(proposal,'failed',this.encounter,event.id);throw Error(`Approved source returned HTTP ${response.status()}`);}
  if(restriction){if(proposal&&this.encounter)this.publicJournal?.transition(proposal,'failed',this.encounter,event.id);this.journal.finish(event,{status:'blocked',reason:restriction,completedAt:new Date().toISOString()});if(new URL(url).hostname==='www.ponsfamily.com'){this.planner.ponsBlocked=restriction;await this.navigate(RESEARCH_PAGES[0],'Pons reports restricted access; approved research fallback.');return;}throw new Error(restriction);}
  ...
  const title=(await this.page!.title()).slice(0,160),tokenAddress=coinAddress(actual),kind=tokenAddress?'coin':actual===P.initial?'explore':new URL(actual).hostname==='ethereum.org'?'knowledge':new URL(actual).hostname==='eth.blockscout.com'?'explorer':'research';
  this.encounter={visitId:`${this.journal.sessionId}:visit-${++this.visit}`,id:actual,label:title,url:actual,kind,enteredAt:new Date().toISOString(),source:'orchestration',tokenAddress,runId:this.runId,transitionId:event.id,reason,proposalId:proposal?.id,selectionOwner:proposal?.owner??'supervisor'};this.publicJournal?.entered(this.encounter,reason,proposal);if(proposal)this.publicJournal?.transition(proposal,'completed',this.encounter,event.id);this.journey.push(this.encounter);this.journey=this.journey.slice(-12);this.planner.entered(actual,this.encounter.visitId);this.visited.add(actual);if(tokenAddress)this.coinVisited.add(tokenAddress.toLowerCase());this.counters.uniquePages=this.visited.size;this.counters.coinPages=this.coinVisited.size;this.encounterDecisions=0;this.unchanged=0;
 ...
 private nextProposal():ResearchProposal {return this.planner.propose(this.encounter!,this.pageText);}
 private narrate(d:ExhibitDecision){
  if(!this.narrator||!this.publicJournal||!this.encounter||this.narratedVisit===this.encounter.visitId)return;this.narratedVisit=this.encounter.visitId;
  const view=this.memory?.view(),recall=view?.recall,memories=(recall?.matches??[]).flatMap(match=>{const item=this.memory?.detail(match.id)?.encounter;return item?[{id:item.id,title:item.title,url:item.url,method:match.methods.join(' + '),outcomes:item.outcomes}]:[];});
  const candidates=[...new Map([...view?.notes.filter(n=>n.creator==='curated-knowledge').map(n=>({id:n.id,url:n.source,title:n.title}))??[],...REFERENCE_PAGES.map((url,i)=>({id:'reference-'+i,url,title:['Wallets and accounts','Transactions','Blocks','Smart contracts','Token interfaces','Block explorers'][i]})),...RESEARCH_PAGES.map((url,i)=>({id:'research-'+i,url,title:'Larval research '+(i+1)})),...[...this.planner.coins].sort((a,b)=>(this.planner.visits.get(a[0])??0)-(this.planner.visits.get(b[0])??0)).slice(0,4).map(([url,title],i)=>({id:'coin-'+i,url,title}))].filter(p=>approvedPage(p.url)&&p.url!==this.encounter!.url).map(p=>[p.url,p])).values()];
  const context:NarrationContext={openQuestions:this.publicJournal.questions(),encounter:{...this.encounter},capturedAt:d.capturedAt,frame:`${d.runId}/${d.imageBefore}`,pageText:this.pageText.slice(0,4500),pageTextSha256:sha256(this.pageText.slice(0,4500)),memories,recallIds:recall?[recall.id]:[],sources:(view?.notes??[]).filter(n=>n.creator==='curated-knowledge').map(n=>({id:n.id,title:n.title,url:n.source,passage:n.support+' '+n.note})),candidates,lastOutcome:d.receipt?{status:d.receipt.status,reason:d.receipt.reason,commandId:d.commandId}:undefined,model:{neurons:d.samples[0]?.activity.length??0,mean:d.motor.mean,contrast:d.motor.contrast}};
  this.publicJournal.recall(this.encounter,context);this.narrator.submit(context);
 }
 private async discover(){this.planner.discover(await this.page!.locator('a[href]').evaluateAll(as=>as.map(a=>({url:(a as HTMLAnchorElement).href,title:(a.textContent??'').slice(0,160)}))),this.encounter?.url);}
 ...
 async close(){await this.stopDisplay();await this.desktop?.close();await this.planner.flush();this.desktop=undefined;this.page=undefined;this.context=undefined;}
 async episode(circuit:Circuit,o:EpisodeOptions){
  ...
  if(this.memory)record.memoryConfig={version:'experience-v1',mode:this.memory.mode,initialAdapter:this.memory.adapter().sha256};
  try{
   await this.open();record.browserVersion=this.desktop!.browser.version();if(!this.encounter){this.encounter={visitId:'resume-'+randomUUID(),id:'resume',url:this.planner.view().lastUrl??P.initial,label:'Previously recorded exploration',kind:'explore',enteredAt:startedAt,source:'orchestration',runId,transitionId:'resume'};const next=this.nextProposal();await this.navigate(next.url,'Resume persistent exploration. '+next.reason,next);}this.startDisplay();
   for(let decision=0;decision<P.decisionWindows;decision++){
    ...
    const dwell=Date.now()-Date.parse(this.encounter!.enteredAt),budget=this.encounter!.kind==='explore'?P.listingDecisions:P.maximumEncounterDecisions;
    if(dwell>=P.maximumDwellMs||dwell>=P.minimumDwellMs&&(this.encounterDecisions>=budget||this.unchanged>=this.retryLimit)){await this.discover();const proposal=this.nextProposal();await this.navigate(proposal.url,proposal.reason,proposal);this.retryLimit=P.maximumUnchanged;}
    ...
    if(this.memory&&this.recalledVisit!==this.encounter!.visitId){this.memory.encounter(this.encounter!,before.png,decision);this.recalledVisit=this.encounter!.visitId;}
    ...
    this.planner.result(this.encounter!.url,receipt.status);this.publicJournal?.outcome(this.encounter!,d);this.narrate(d);
  ...
  finally{if(record.incompleteAction)record.incompleteAction.events=this.events.filter(e=>e.commandId===record.incompleteAction!.commandId);this.commandId=null;await this.stopDisplay();this.journal.cancelRun(runId,record.error??'Recording segment ended');await this.memory?.flush();await this.planner.flush();}
