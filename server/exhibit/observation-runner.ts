...
export async function runObservationEpisode(circuit:Circuit,o:EpisodeOptions){
 ...
 try{
  ...
  await main.bringToFront();await desktop.present(main);await main.mouse.move(desktop.pageSize.width/2,desktop.pageSize.height/2);record.setup.events=events.slice();
