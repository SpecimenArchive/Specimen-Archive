...
export interface RunOptions {root:string;seed:number;intervention:BrowserIntervention;fast?:boolean;onUpdate?:(live:BrowserLive)=>void}
export async function runBrowserTrial(circuit:Circuit,options:RunOptions){
  ...
  const origin=executionOrigin(runId),decisions:BrowserDecision[]=[];
  ...
  let browser:Awaited<ReturnType<typeof chromium.launch>>|undefined,context:Awaited<ReturnType<NonNullable<typeof browser>['newContext']>>|undefined;
  ...
  try{
    ...
    const video=page.video(),controller=new BrowserController(circuit,runId,startedAt,options.intervention),executor=new BrowserExecutor(page);
    ...
    for(let decision=0;decision<C.decisions;decision++){
      ...
      const calculation=await controller.observe(png,decision,async(snapshot,input)=>{
        if(!options.fast){const due=windowStart+(snapshot.seq-decision*C.modelStepsPerDecision)/C.modelStepsPerDecision*C.minimumWindowWallMs;await delay(Math.max(0,due-performance.now()));}
    ...
    await context.close();context=undefined;await video?.saveAs(join(directory,'browser.webm'));await video?.delete();
  }catch(e){record.error=(e as Error).message;record.outcome='error';}
  finally{await context?.close();await browser?.close();await new Promise<void>(r=>fixture.close(()=>r()));}
