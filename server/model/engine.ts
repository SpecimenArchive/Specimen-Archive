import type { Circuit, ModelEvent, Pose, Snapshot, LightCondition } from '../../shared/types';
...
export class Engine {
  ...
  events: ModelEvent[]=[];
  condition?: LightCondition & {epoch:number};
  private lastSensory?: {left:number;right:number};
  ...
  step(override?:{left:number;right:number}){
    const env=environmentAt(this.time,this.pose,this.condition);
    if(env.epoch!==this.epoch){this.epoch=env.epoch;this.responded=false;this.event('environment',env.label);}
    this.lastSensory={left:override?.left??env.lightLeft,right:override?.right??env.lightRight};
    this.network.step(this.lastSensory.left,this.lastSensory.right);
  ...
  snapshot(runId:string,seq:number,startedAt:string,wallElapsed:number): Snapshot {
    const environment=environmentAt(this.time,this.pose,this.condition);
