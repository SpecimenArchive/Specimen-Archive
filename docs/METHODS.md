...
The observation plate and registered motion fields form the optical instrument. Preparation records and image provenance are retained with the assets and described in the [assessment companion](ASSESSMENT_COMPANION.md#image-preparation-record). The renderer keeps physical proportions across viewport sizes, interpolates received states and never extrapolates active motion after a signal loss. Browser mode uses the controller's own neural state and accelerated model time; the original light experiment uses 0.5× wall time. View and neuron-selection controls cannot alter either model.
...
The rate circuit computes motor activity. The memory store retains and retrieves encounters, while the journal’s separate OpenAI narrator develops source-linked commentary and research proposals. The supervisor accepts eligible destinations and records navigation; wheel commands remain with the neural decoder. Project configuration and public source documents are indexed directly. The station profile records the current circuit, capabilities and operating limits.
...
The [annelid study](https://elifesciences.org/articles/97964) discusses developing mushroom-body circuits separately from the selected visual module; it does not establish this software update as biological plasticity. No fruit-fly learning rule was transplanted. Curated knowledge uses short checked passages from primary Ethereum and eLife documentation with actual retrieval timestamps and HTML checksums. The [reference project’s voice description](https://github.com/fruitflydev/flycoinrh#the-voice) documents the same separation between narration and neural computation.
