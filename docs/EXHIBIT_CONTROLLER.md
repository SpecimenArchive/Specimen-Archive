...
WebSocket packets are authoritative and numbered independently of model steps. A reconnect receives the current session. The specimen, signal panel, browser, apparatus monitor and selected event all share the same observation context. The monitor uses a projective texture mapping into a fixed generated bench. Geometry, optical effects and generated tissue are presentation, not additional telemetry. A stale/disconnected observer freezes at its last state.
