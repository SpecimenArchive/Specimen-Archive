...
The committed Windows evidence also replayed exactly in both Windows and Linux CI: [final evidence workflow](https://github.com/SpecimenArchive/Specimen-Archive/actions/runs/34610356098). A fresh local runtime serves the bundled publication receipts after validating their record hashes, so another developer sees the actual GitHub links without needing this machine?s private runtime.
