...
mkdirSync('docs/results',{recursive:true});writeFileSync(process.env.EXHIBIT_SUITE_REPORT||(development?'runtime/exhibit-development.json':'docs/results/exhibit-heldout.json'),JSON.stringify({createdAt:new Date().toISOString(),sessionId,config:C,protocol:development?'Development layout, excluded from held-out results':'Frozen profile evaluated on three held-out cases; same cases under both interventions. Failures retained.',rows},null,2)+'\n');
