...
for(const record of calibration.records){
  ...
  const result=await page.evaluate(async data=>{
    ...
    renderer.bench(await load('/assets/apparatus-master-v4-marker.png'));renderer.draw(await load(data));const png=canvas.toDataURL();renderer.dispose();return png;
  },data);
  const bytes=Buffer.from(result.split(',')[1],'base64');writeFileSync(`${out}/composite-${record.theme}.png`,bytes);
  const composed=PNG.sync.read(bytes),source=PNG.sync.read(readFileSync('public/assets/apparatus-master-v4-marker.png'));let changedOutside=0;
