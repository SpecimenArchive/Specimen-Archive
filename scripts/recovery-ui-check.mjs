...
const base=process.env.EXHIBIT_URL||'http://127.0.0.1:4317',out='runtime/recovery-review';mkdirSync(out,{recursive:true});
...
try{
  ...
  const result={checkedAt:new Date().toISOString(),backendSourceRevision:live.sourceRevision,backendSourceDirty:live.sourceDirty,sessionId:live.sessionId,captureSource:live.desktop?.source,frontend:'Recovery working tree built locally; backend is the preserved running revision.',checks,workstation:{capturesRequested:0,embeddedCaptures:0,liveData:true},mutationHTTP:mutation.status,errors};
  writeFileSync('docs/results/recovery-ui.json',JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result,null,2));assert.equal(errors.length,0);
