...
**Current observation exhibit:** a continuous intact controller captures the visible Chrome page on a Windows 11 VM, computes the published circuit, then sends decoded wheel commands. The large apparatus, specimen, sensory/neural instruments and inspectable activity feed share one backend session. The separately preserved benchmark still tests cursor movement, scrolling and link/button activation. See [STATUS.md](STATUS.md), [the observation upgrade](docs/OBSERVATION_UPGRADE.md), [the companion worksheet](docs/WORKSHEET.html) and the [frozen baseline](docs/evidence/browser/README.md).

## Public Windows observation

**Live observer: https://specimenarchive.com**. The existing By-Hoster Windows 11 VM runs genuine Chrome, native desktop capture, the neural backend, persistent experience memory, video finalization and a separate SYSTEM evidence publisher. Cloudflare provides HTTPS and the outbound tunnel. The home PC runs no exhibit service. RDP is disconnected; a cold reboot still requires a manual console login.

The active profile explores Pons listings and discovered coin detail pages, with occasional published larval research. It never browses its own observer dashboard. Wheel direction comes from captured pixel contrast propagated through the unchanged 47-cell rate model. Page selection and fixed pointer placement are explicitly recorded supervisor actions. Memory stores meaningful encounters and retrieves prior evidence; a separate bounded retry adapter is tested against fixed and disabled baselines. No LLM or wallet operation is enabled.

See [the fixed-source control audit](docs/EXTERNAL_CONTROL_AUDIT.md), [public deployment and service locations](docs/PUBLIC_DEPLOYMENT.md), [memory protocol and results](docs/MEMORY_EXPERIMENT.md), and [current status](STATUS.md). Historical local benchmarks below are preserved reproduction paths, not the active public browsing schedule.
...
Original code: [MIT](LICENSE). Derived research data: **CC BY 4.0**, with separate attribution and transformations. Fonts and dependencies retain their licences in [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md). Raw research cache, dependencies, credentials and runtime logs are ignored. Tested source milestones are published to the existing SpecimenArchive/Specimen-Archive master branch; compact evidence is published separately to specimen-records. Runtime services are deployed on the user-provisioned VM. No transaction, signing action or social account has been created.
