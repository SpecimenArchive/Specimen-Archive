...
The By-Hoster VM runs the observation website/backend, genuine Chrome/native capture worker, local recordings and a separate SYSTEM evidence publisher. Windows 11 build 26200, clean source `7f72d2b`, real scrolling, exact replay and saved page recordings are verified. SSH uses the original trusted host key; credentials stay outside Git. The private observer preview is **http://127.0.0.1:4319/** through the existing SSH tunnel. The home PC runs no exhibit service. RDP is disconnected. See the upgrade report for the final endurance/recovery result and the cold-reboot login limitation.
...
Inside the VM, **http://127.0.0.1:4317/?display=workstation** suppresses recursive captures while keeping live signals and events. The pinned dashboard receives 38 of 48 neural windows, with five each on the approved eLife reference and local worksheet. Page focus is recorded orchestration; wheel direction comes from the documented pixel encoder and unchanged neural decoder. The model does not understand page content or control arbitrary websites.
