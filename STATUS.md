...
- Next: establish the initial RDP login and then SSH; read private `%LOCALAPPDATA%/SpecimenArchive/connection.json` without displaying credentials; verify host/OS and deploy exact tested code to By-Hoster. Keep the old local session until the remote session works, then stop it through its owning terminal. Complete native tab pinning, public/project task activity review, real Windows raw/composited recording, ten-minute two-observer/RDP-off check and restart check. Public hosting/TLS remains separate from private SSH-forwarded preview.
