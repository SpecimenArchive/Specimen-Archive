...
test('visible upper/lower contrast passes through the same circuit to opposite scroll commands',async()=>{
 ...
 const commands=[];for(const png of [up,down]){const controller=new ExhibitController(circuit,'contrast','2026-01-01T00:00:00Z');const result=await controller.observe(png,0,undefined,'observation-contrast-v2');assert.equal(result.command.kind,'scroll');commands.push(result.command.wheelY);}
 assert.deepEqual(commands,[-48,48]);
 for(const intervention of ['clamp-all-motors','disconnect-photoreceptors'] as const){const controller=new ExhibitController(circuit,'control','2026-01-01T00:00:00Z',intervention);const result=await controller.observe(down,0,undefined,'observation-contrast-v2');assert.equal(result.command.kind,'wait');}
});
...
test('real workstation page contrast retains a usable direction without saturating both sensory channels',async()=>{
 const png=readFileSync('docs/evidence/observation/retina-workstation.png');
 const old=await new ExhibitController(circuit,'retina-v1','2026-01-01T00:00:00Z').observe(png,0,undefined,'observation-contrast-v1');
 const current=await new ExhibitController(circuit,'retina-v2','2026-01-01T00:00:00Z').observe(png,0,undefined,'observation-contrast-v2');
 assert(old.input.right>old.input.left);assert.equal(old.command.wheelY,-48);
 assert(current.input.right>current.input.left);assert.equal(current.command.wheelY,48);
});
