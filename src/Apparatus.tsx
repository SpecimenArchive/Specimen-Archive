...
export function Apparatus({live,desktopImage,health}:{live:ExhibitLive|null;desktopImage:HTMLImageElement|null;health?:string}){
  ...
  return <section className={`apparatus-panel ${expanded?'station-expanded':''}`} data-run-id={live?.runId} data-model-step={live?.snapshot?.seq} data-desktop-frame={(live?.display??live?.desktop)?.path}>
    ...
    <div className="apparatus-body"><div className="apparatus-image"><canvas width={screenPlacement.width} height={screenPlacement.height} ref={canvas} aria-label="Laboratory workstation: the Windows desktop capture appears in the apparatus monitor"/>{error&&<p role="alert">{error}</p>}</div>
