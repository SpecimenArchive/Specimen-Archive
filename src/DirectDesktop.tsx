...
export function DirectDesktop({live,desktopImage,health}:{live:ExhibitLive|null;desktopImage:HTMLImageElement|null;health:string}){
 ...
 return <section className={`direct-desktop panel ${expanded?'desktop-expanded':''}`} data-run-id={live?.runId} data-model-step={live?.snapshot?.seq} data-desktop-frame={capture?.path}>
  <div className="panel-heading"><span>B / WINDOWS 11 Â· DIRECT VIEW</span><button className="quiet-button" onClick={()=>setExpanded(!expanded)}>{expanded?'Close Ã—':'Expand â†—'}</button></div>
  ...
  <div className="desktop-foot"><span>{health==='replay'?'RECORDED':health==='live'?'CAPTURED':'HELD'} Â· {capture?.capturedAt.slice(11,23)??'â€”'} UTC</span><span>{capture?`${capture.width} Ã— ${capture.height}`:'WINDOWS CAPTURE'}</span></div>
 </section>;
}
