...
export function DirectDesktop({live,desktopImage,health,opening=false}:{live:ExhibitLive|null;desktopImage:HTMLImageElement|null;health:string;opening?:boolean}){
 const canvas=useRef<HTMLCanvasElement>(null),[expanded,setExpanded]=useState(false),revealed=useRef(false),capture=live?.display??live?.desktop,r=live?.lastAction?.receipt??live?.receipt;
 if(!opening)revealed.current=true;
 ...
 return <section className={`direct-desktop panel ${expanded?'desktop-expanded':''}`} data-run-id={live?.runId} data-model-step={live?.snapshot?.seq} data-desktop-frame={capture?.path}>
  ...
  <div className="desktop-containment"><div className={'desktop-pixels'+(revealed.current?'':' awaiting-capture')} aria-busy={!revealed.current} style={{aspectRatio:capture?`${capture.width}/${capture.height}`:'1280/800'}}>
   ...
   {!revealed.current&&<span className="station-opening">Opening the station…</span>}
  </div>{revealed.current&&!desktopImage&&<p className="desktop-wait">Awaiting a verified desktop capture</p>}</div>
