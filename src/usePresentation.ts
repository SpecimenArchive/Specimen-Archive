...
export function usePresentation(live:ExhibitLive|null,suppressCaptures=false){
  const key=live?`${live.runId}/${live.browserFrame}/${live.desktop?.path??''}`:'',requested=useRef('');requested.current=key;
  ...
  useEffect(()=>{
    if(suppressCaptures||!live?.browserFrame){setPair(null);return;}
    let cancelled=false;
    Promise.all([image(live.browserFrame),live.desktop?image(`${live.runId}/${live.desktop.path}`):Promise.resolve(null)]).then(([,desktop])=>{
  ...
  if(suppressCaptures||!live?.browserFrame)return {live,desktopImage:null,pending:false};
