...
export function useStream(){
  ...
  const [browser,setBrowser]=useState<BrowserLive|null>(null);
    ...
    function connect(){
      ...
      socket.onmessage=e=>{
        try{const p=JSON.parse(e.data) as StreamPacket;const s=p.snapshot;if(s?.version!==1||!Number.isFinite(s.modelTime)||s.activity.some(v=>!Number.isFinite(v)))return;
          ...
          if(latest.current?.runId===s.runId&&s.seq===latest.current.seq){if(p.browser)setBrowser(p.browser);return;}
          if(latest.current?.runId!==s.runId)history.current=[];
          previous.current=latest.current;latest.current=s;received.current=performance.now();history.current.push(s);if(history.current.length>600)history.current.shift();setSnapshot(s);setBrowser(p.browser??null);setHealth('live');attempt=0;
  ...
  return {snapshot,browser,health,latest,previous,received,history};
}
