...
interface ObserverStart {cached:{live:ExhibitLive;savedAt:number;desktop:string;input:string|null}|null;current:ExhibitLive|null;decoded:Map<string,HTMLImageElement>;image:(path:string)=>Promise<HTMLImageElement>;ready:Promise<ExhibitLive|null>}
...
export function savePresentation(pair:DecodedPresentation|null){
 ...
 try {const encode=(image:HTMLImageElement|null)=>{if(!image)return null;const c=document.createElement('canvas');c.width=image.naturalWidth;c.height=image.naturalHeight;c.getContext('2d')!.drawImage(image,0,0);return c.toDataURL('image/png');};
  sessionStorage.setItem('specimen-observer-frame-v1',JSON.stringify({savedAt:Date.now(),live:pair.live,desktop:encode(pair.desktop),input:encode(pair.input)}));
