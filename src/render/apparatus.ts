...
export function createApparatusRenderer(canvas:HTMLCanvasElement){
  ...
  const fs=shader(gl.FRAGMENT_SHADER,`#version 300 es
    ...
    uniform mat3 inverse; uniform vec2 sourceSize; uniform vec2 renderSize; uniform float available;
    void main(){
      ...
      // Register the complete native image directly to the four glass corners.
      // A second contain/cover operation would insert strips or discard pixels.
      vec3 display=texture(desktop,clamp(q,vec2(0.),vec2(1.))).rgb;
      // Fixed photographed reflection follows the glass; no full-display blur.
      display=display*.87+photo*.08+vec3(.004);
      outputColor=vec4(mix(photo,display,glass*available),1.);
    }`);
  ...
  gl.uniform2f(gl.getUniformLocation(program,'sourceSize'),P.width,P.height);
